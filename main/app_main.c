...
// Unified write callback for the Water Pump device.
// RainMaker devices support only one write callback, so both the pump
// Power parameter and the Watering Schedule parameter are handled here.
    ...
    esp_rmaker_write_ctx_t *ctx)
{
    ...
    // Handle watering schedule parameter updates
    if (param == watering_schedule_param) {
        if (val.type == RMAKER_VAL_TYPE_STRING) {
            if (val.val.s == NULL || strlen(val.val.s) >= 128) {
                ESP_LOGE(TAG, "Schedule string too long, ignoring");
            ...
            int i = 0;
            memset(watering_times, 0, sizeof(watering_times));
    ...
    // Handle pump power parameter (voice / app toggle)
...
void sensor_task(void *arg) {
    ...
    while (1) {
        ...
        if (dht11_read(CONFIG_DHT11_PIN, &dht11_reading) == ESP_OK) {
            temperature = dht11_reading.temperature;
...
void app_main(){
    ...
    pump_param = esp_rmaker_param_create("Power", ESP_RMAKER_PARAM_POWER, esp_rmaker_bool(false), PROP_FLAG_READ | PROP_FLAG_WRITE | PROP_FLAG_PERSIST);
    ...
    xTaskCreate(t1_drysoil, "t1_drysoil", 4096, NULL, 5, &receiverTaskHandle);
    xTaskCreate(t2_schedule, "t2_schedule", 4096, NULL, 5 ,NULL);
    xTaskCreate(t3_pushbutton, "t3_pushbutton", 4096, NULL, 5 ,NULL);
